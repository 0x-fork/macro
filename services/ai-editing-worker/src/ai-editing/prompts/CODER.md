...
- Do the whole task in ONE `runCode` call. The snippet is arbitrary JavaScript, so any number of `editor.*` calls belong in the same call — splitting them across calls costs a round trip and buys nothing. Additional calls are for recovering from an error, and you get very few of them.
