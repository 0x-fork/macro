...
- `CS-##` — cloud storage (`rust/cloud-storage`)
- `FE-##` — frontend (`js/app`, `js/lexical-core`)
...
## Cloud storage (`rust/cloud-storage`)
...
## Frontend (`js/app`, `js/lexical-core`)
...
- **FE-25** `[ui]` Semantic color tokens, not raw Tailwind palette classes — the default
  palette is disabled via `--color-*: initial` in `js/app/packages/app/index.css`, so
