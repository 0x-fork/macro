...
/**
 * The small vent arcs — part of the body mass, drawn on top of it (as in the
 * source art) so the filled tile doesn't hide them. They thrust with the body
...
/**
 * The full isometric tile (back + body), minus its logo. Drawn at the
 * template's original coordinates (face centered at (427.5, 168.3)); position
 * a module by transforming the wrapping group.
 */
...
/**
 * Shrink transform for a mark already drawn in the face's perspective at full
 * template scale (e.g. Linear's hand-drawn logo), to match the visual
 * footprint of the flat logos placed via {@link LOGO_FACE_TRANSFORM}.
 */
export const LINEAR_LOGO_TRANSFORM =
 ...
 * face transform; pass {@link LINEAR_LOGO_TRANSFORM} for a mark already in
 * perspective).
 */
...
// thrusts +D while the rings stay put; the shaft streaks −D behind it.
...
// The accent shaft: a rect laid along the −D axis via SHAFT_TRANSFORM (local
...
const SHAFT_TRANSFORM = 'translate(420 162) rotate(223.5)';
const SHAFT_LENGTH = 230;
const SHAFT_HALF = 15; // half-width of the thin (peak) shaft

// Shaft: starts wide and faint, focuses to a thin, bright streak at the peak
 ...
 * `var(--…)` keyframe). The var is global, so a throwaway probe on the body
 * resolves it through the cascade.
 */
const resolveAccent = () => {
  const probe = document.createElement('span');
  probe.style.cssText =
    'color: var(--color-accent); position: absolute; visibility: hidden';
 ...
 * `setup-graphic` class for styling; see GraphicsPreview for a standalone
 * host). Ambient motion is self-driven with WAAPI so it survives a parent's
 * DOM re-attach without restarting — a CSS animation would visibly pop. Pass
 * `phaseMs` (a negative offset) so several modules don't move in unison.
 *
 ...
 * accent shaft focuses in behind it — a quick ramp, a brief hold, then a slow
 * settle back.
 *
...
export function Module(props: {
  ...
  const shaftGradientId = createUniqueId();
  ...
  // and vents are `currentColor`, so setting/animating their group's `color`
  // recolors them: downloading pulses the logo ink↔accent; linked holds both
  // the logo and the vents accent.
  createEffect(() => {
    ...
    if (state === 'linked') {
      ...
      logoGroup.style.color = accent;
      if (ventsGroup) ventsGroup.style.color = accent;
    } else if (state === 'downloading' && !prefersReducedMotion()) {
      const ink = getComputedStyle(root).color;
      const accent = resolveAccent();
      logoAnimation = logoGroup.animate(
        [{ color: ink }, { color: accent, offset: 0.5 }, { color: ink }],
        {
          duration: 1600,
  ...
  onMount(() => {
    ...
    for (const ring of root.querySelectorAll<SVGElement>('.st4, .st6')) {
      animations.push(
  ...
  // pop; the body and shaft have no ambient motion, so they just replace.
  const accelerate = () => {
    ...
    const large = root.querySelector<SVGElement>('.st6');
    const small = root.querySelector<SVGElement>('.st4');
    const shaft = root.querySelector<SVGElement>('.module-shaft-scale');
    ...
    if (shaft) clickAnimations.push(shaft.animate(ACCEL_SHAFT, dur));
    for (const vent of root.querySelectorAll<SVGElement>('.st1')) {
      ...
      <g class="module" transform={props.place}>
        <defs>
          <linearGradient
            ...
            y2="0"
          >
            <stop offset="0%" class="module-shaft-hot" />
            <stop offset="100%" class="module-shaft-cold" />
          </linearGradient>
        </defs>
        {/* Accent shaft behind the module — hidden until the click burst. The
            inner group's width (scaleY) and opacity are what animate. */}
        <g class="module-shaft" transform={SHAFT_TRANSFORM}>
          <g class="module-shaft-scale">
            <rect
              x="0"
              y={-SHAFT_HALF}
              width={SHAFT_LENGTH}
