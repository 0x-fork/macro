...
export const SoupView = (props: SoupViewProps) => {
  ...
  // A fresh Inbox or Channels view starts in preview mode once its initial
  // result settles, but only when it has an entity to show. Resolve this once
  // so manually exiting preview mode is not undone by later Soup updates.
  let initialPreviewResolved = false;
  createEffect(() => {
    if (initialPreviewResolved || soupView.source.isLoading()) return;
    initialPreviewResolved = true;
    if (contentId !== 'inbox' && contentId !== 'channels') return;
