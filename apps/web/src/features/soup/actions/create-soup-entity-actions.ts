...
export function createSoupEntityActions() {
  ...
  const buildActionGroups: BuildActionGroups = (
     ...
     *
     * Content already mounted in another split is skipped. The Preview Pair's
     * own Viewer is the exception because opening supersedes that preview.
     */
    const openableEntity = (): EntityData | undefined => {
      ...
      if (existing && existing.id !== panel.handle.viewerId()) return undefined;
      return entity;
    };
