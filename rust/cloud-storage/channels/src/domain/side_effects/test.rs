...
fn bot_mentions_recognize_bot_and_system_bot_user_tags() {
    ...
    let task_agent = bot_id::TASK_AGENT_BOT_ID.into_storage_id().to_string();
...
fn bot_mentions_reject_bare_uuid_ids() {
    ...
    let mentions = vec![
        ...
        mention("user", &bot_id::TASK_AGENT_BOT_ID.as_uuid().to_string()),
...
fn system_bot_user_mention_is_not_a_user_recipient() {
    ...
    assert!(is_bot_user_mention(&mention(
        "user",
        bot_id::TASK_AGENT_BOT_ID.into_storage_id().as_ref()
    )));
