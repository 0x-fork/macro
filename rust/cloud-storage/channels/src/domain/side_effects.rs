...
fn bot_mention_ids(mentions: &[SimpleMention]) -> Vec<BotId> {
    ...
    mentions
        ...
        .filter_map(|mention| match mention.entity_type.as_str() {
            ...
            "user" => mention_bot_id(&mention.entity_id).filter(|id| bot_id::is_system_bot(*id)),
            _ => None,
        })
...
/// never `bot|<uuid>` principals, so this only matches the system bots
/// surfaced through the user-mention UI.
fn is_bot_user_mention(mention: &SimpleMention) -> bool {
    mention.entity_type == "user"
        && mention_bot_id(&mention.entity_id).is_some_and(bot_id::is_system_bot)
}
