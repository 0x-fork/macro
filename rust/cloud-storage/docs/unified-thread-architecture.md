...
**`models_comms` crate** (new leaf, following the `models_*` convention): the single home for the parent-agnostic message vocabulary — `CountedReaction`, `MessageAttachment`, `SimpleMention`. `channels`, `message_threads`, and `comms_db_client` all consume these via re-exports at their old paths, so no call sites changed. Message-*shape* types (`ChannelMessage` vs `ThreadMessage`, `ResolvedChannelMessage` vs `ResolvedThreadMessage`, …) intentionally remain per-crate until the channel paths delegate to `ThreadService`, because the channel variants encode a non-optional `channel_id` that flows through channels' API responses; folding them together is the convergence-phase change, not a type move. (The private `*RowDb` structs in the Postgres repos are sqlx `query_as!` binding artifacts, same pattern as channels' repo — not domain duplication.)
