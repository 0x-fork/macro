...
async function fetchChannelPreviews(
  ...
  return data.previews.map((channel) => {
    ...
    return match(channel)
      .returnType<PreviewItem>()
      .with({ type: 'access' }, (channel) => ({
        ...base,
      ...
      .with({ type: 'no_access' }, { type: 'does_not_exist' }, (channel) => ({
        ...base,
        access: channel.type,
        loading: false,
      }))
...
async function fetchDocumentPreviews(ids: string[]): Promise<PreviewItem[]> {
  ...
  return data.previews.map((doc) => {
    ...
    return match(doc)
      ...
      .with({ type: 'no_access' }, { type: 'does_not_exist' }, (doc) => ({
        ...base,
        access: doc.type,
        loading: false,
      }))
...
async function fetchChatPreviews(ids: string[]): Promise<PreviewItem[]> {
  ...
  return data.previews.map((chat) => {
    ...
    return match(chat)
      ...
      .with({ type: 'no_access' }, { type: 'does_not_exist' }, (chat) => ({
        ...base,
        access: chat.type,
        loading: false,
      }))
