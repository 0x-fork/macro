...
export function ThreadReplyButton(props: ThreadReplyButtonProps) {
  ...
  return (
    <button
      type="button"
      class={cn(
        'w-min mb-2 border border-edge-muted bg-surface hover:bg-hover hover-transition-bg flex flex-row justify-center items-center size-(--user-icon-width) touch:min-h-(--user-icon-width) touch:min-w-(--user-icon-width) text-ink-muted rounded-md',
        local.class
      )}
      ref={(el) => {
