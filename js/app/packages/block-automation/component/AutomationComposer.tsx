...
export function AutomationComposer() {
	...
	// Reset the draft every time the dialog opens so users get a clean slate.
	createEffect(
		on(automationComposerOpen, (open) => {
			if (open) setRawDraft(createEmptyDraft());
		}),
	);
	...
	const handleCreate = () => {
		...
		if (error) {
			toast.alert("Missing required fields", error);
			return;
		}
				...
				<DialogWrapper width="640px">
					<div class="flex cursor-default flex-col text-ink">
						...
						<div class="grid max-h-[70vh] gap-3 overflow-y-auto px-3 py-3">
							...
							<div class="grid gap-3 border border-edge-muted p-3">
								<div>
						...
						<div class="flex items-center justify-end gap-2 border-t border-edge-muted px-3 py-2">
							...
							<Button
								...
								class="cursor-default"
								disabled={
									Boolean(formError()) || createMutation.isPending
								}
