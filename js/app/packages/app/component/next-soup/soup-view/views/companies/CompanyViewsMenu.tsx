...
export function CompanyViewsMenu(props: {
  ...
  const { soup, searchText, setSearchText, activeTab, setActiveTab } =
    useSoupView();
  ...
  const captureCurrentView = (): CrmViewConfig => ({
    kind: 'crm',
    // Facet selection carries the full filter state (Stage/Owner included).
    // serialize() already returns a JSON-safe snapshot.
    facets: soup.facets.serialize(),
    clientFilters: {
  ...
  // Mirrors the init/applyTabPreset path: replace the facet selection +
  // predicates atomically. Stage/Owner sub-filters ride in `facets`
  // (`company-stage`/`company-owner`), so hydrating covers them.
  const applyView = (config: CrmViewConfig) => {
    batch(() => {
      soup.facets.hydrate(config.facets ?? {});
