...
export const SoupFilterToolbar: Component<SoupFilterToolbarProps> = (props) => {
  ...
  const availableFilterSuggestions = createMemo((): FilterSuggestion[] => {
    ...
    for (const filter of contextualFilters()) {
      if (!activeContextualIds.has(filter.id)) {
        const category =
          CATEGORY_LABELS[filter.category ?? 'other'] ??
          filter.category ??
          'Other';
    ...
    for (const suggestion of contextualSuggestions) {
      if (
        (seenLabels.get(suggestion.label) ?? 0) > 1 &&
  ...
  const filteredSuggestions = createMemo(() => {
    ...
    return availableFilterSuggestions().filter(
      (s) =>
  ...
  const quickFilterGroups = createMemo(() => {
    ...
    const activeIds =
      props.activeFilterIds ?? (soup.filters.activeIds() as FilterID[]);
                      ...
                      {(filter) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              handleQuickFilterClick(filter.id, group)
                            }
                          >
                            {filter.label}
                          </button>
              ...
              <div class="p-3">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <For each={TYPE_FILTERS}>
                    {(filter) => {
                      const isActive = () =>
                        soup.filters.activeIds().includes(filter.id);
                      return (
                        <button
                          type="button"
                          ...
                          classList={{
                            ...
                            'bg-panel-muted text-ink-muted hover:bg-ink/10 hover:text-ink':
                              !isActive(),
                          }}
 ...
 */
const ContextualFilterSection: Component<ContextualFilterSectionProps> = (
  props
) => {
  const activeCount = createMemo(() => {
    return props.filters.filter((f) =>
      props.contextualFilterState.isActive(f.id)
    ).length;
  });
