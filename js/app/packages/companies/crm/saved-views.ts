...
export type CrmViewConfig = {
  kind: 'crm';
  /** Facet selection (facet id → selected option ids). The soup view's
   * filter state, including the Stage/Owner sub-filters. */
  facets?: Record<string, string[]>;
  /** @deprecated Server query filters (legacy pre-facet views). */
  filters?: unknown;
  ...
  sort?: string[];
  viewMode?: 'list' | 'board';
  /** @deprecated Stage ids — now part of `facets` (`company-stage`). */
  stageFilter?: string[];
  /** @deprecated Owner ids — now part of `facets` (`company-owner`). */
  ownerFilter?: string[];
  activeTab?: string;
};
