...
use super::models::{
    AgentConfig, AgentWebhook, AuthenticatedBot, Bot, BotChannel, BotId, BotOwner, BotToken,
    BotTokenCandidate, CreateBotRequest, CreateBotResponse, CreateBotTokenRequest,
...
pub trait BotRepo: Clone + Send + Sync + 'static {
    ...
    /// Create an owned bot with the supplied id.
    ///
    /// The id is generated by the service so an external agent's webhook can
    /// be provisioned (filtered to the bot id) before the row is inserted.
    fn create_owned_bot(
        &self,
        bot_id: BotId,
        owner: BotOwner,
...
/// Request to provision a webhook for an external agent bot.
#[derive(Debug, Clone)]
pub struct ProvisionAgentWebhookRequest {
    /// Owner of the agent bot; the webhook lands in this owner's workspace.
    pub owner: BotOwner,
    /// Agent bot id used to filter deliveries.
    pub bot_id: BotId,
    /// Bot display name used to label the webhook.
    pub bot_name: String,
    /// Endpoint URL deliveries are sent to.
    pub endpoint_url: String,
}

/// Error provisioning or removing an external agent's webhook.
#[derive(Debug, thiserror::Error)]
pub enum AgentWebhookError {
    /// The supplied endpoint configuration was rejected.
    #[error("{0}")]
...
/// Provisions delivery webhooks for external agent bots.
///
/// Implemented over the webhook management service at the composition root.
pub trait AgentWebhookProvisioner: Clone + Send + Sync + 'static {
    /// Provision a webhook subscribed to the agent's events, filtered to the
    /// agent bot id, in the bot owner's workspace.
    fn provision(
        &self,
        caller: MacroUserIdStr<'static>,
        req: ProvisionAgentWebhookRequest,
    ) -> impl Future<Output = Result<AgentWebhook, AgentWebhookError>> + Send;

    /// Remove a previously provisioned webhook.
    fn remove(
        &self,
        caller: MacroUserIdStr<'static>,
        webhook_id: String,
    ) -> impl Future<Output = Result<(), AgentWebhookError>> + Send;
}
...
pub trait BotService: Clone + Send + Sync + 'static {
    ...
    fn create_bot(
        ...
        caller: MacroUserIdStr<'static>,
        req: CreateBotRequest,
    ) -> impl Future<Output = Result<CreateBotResponse, BotError>> + Send;
