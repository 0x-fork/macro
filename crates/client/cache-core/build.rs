//! Build-time schema metadata codegen.
//!
//! Parses the in-repo SDL schema and emits `$OUT_DIR/schema_meta.rs`: static
//! lookup tables consumed by `src/meta.rs`.
//!
//! Key policy is the **presence-of-id convention**: an output object type
//! with an `id: ID!` field is a normalized entity keyed by
//! `__typename:id`; a type without `id` is embedded inline in its parent
//! record. The schema itself is the policy — there is no client-side key
//! config to drift. The build still fails on malformed shapes (e.g. a
//! nullable or non-ID `id`).

// std::env::var is disallowed workspace-wide to force `macro_env_var` for
// service config; build scripts read cargo-provided vars (CARGO_MANIFEST_DIR,
// OUT_DIR), which have nothing to do with app secrets.
#![allow(clippy::disallowed_methods)]

use apollo_compiler::Schema;
use apollo_compiler::schema::ExtendedType;
use sha2::{Digest, Sha256};
use std::collections::{BTreeMap, BTreeSet};
use std::fmt::Write as _;
use std::path::Path;

const SCHEMA_PATH: &str = "../../../static_assets/schema.graphql";

fn main() {
    println!("cargo:rerun-if-changed={SCHEMA_PATH}");

    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let schema_path = Path::new(&manifest_dir).join(SCHEMA_PATH);
    let schema_text = std::fs::read_to_string(&schema_path)
        .unwrap_or_else(|e| panic!("cannot read {}: {e}", schema_path.display()));

    let schema = Schema::parse_and_validate(&schema_text, "schema.graphql")
        .unwrap_or_else(|e| panic!("schema does not validate: {e:?}"));

    let query_root = schema
        .schema_definition
        .query
        .as_ref()
        .expect("schema has no query root")
        .name
        .to_string();
    let mutation_root = schema
        .schema_definition
        .mutation
        .as_ref()
        .map(|m| m.name.to_string());

    let mut errors: Vec<String> = Vec::new();
    let mut entries: Vec<(String, String)> = Vec::new(); // (type name, TypeMeta literal)

    // Interface implementors (reverse map).
    let mut implementors: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
    for (name, ty) in schema.types.iter() {
        if let ExtendedType::Object(obj) = ty {
            for iface in &obj.implements_interfaces {
                implementors
                    .entry(iface.name.to_string())
                    .or_default()
                    .insert(name.to_string());
            }
        }
    }

    for (name, ty) in schema.types.iter() {
        let name = name.to_string();
        if name.starts_with("__") {
            continue; // introspection machinery
        }
        match ty {
            ExtendedType::Object(obj) => {
                let is_operation_root = name == query_root || Some(&name) == mutation_root.as_ref();
                let key_literal =
                    key_literal(&name, is_operation_root, obj.fields.iter(), &mut errors);
                let fields = fields_literal(&schema, obj.fields.iter(), &mut errors);
                entries.push((
                    name.clone(),
                    type_meta_literal(&name, "Object", &key_literal, &fields, &[]),
                ));
            }
            ExtendedType::Interface(iface) => {
                let key_literal = key_literal(&name, false, iface.fields.iter(), &mut errors);
                let fields = fields_literal(&schema, iface.fields.iter(), &mut errors);
                let possible: Vec<String> = implementors
                    .get(&name)
                    .map(|s| s.iter().cloned().collect())
                    .unwrap_or_default();
                entries.push((
                    name.clone(),
                    type_meta_literal(&name, "Interface", &key_literal, &fields, &possible),
                ));
            }
            ExtendedType::Union(u) => {
                let possible: Vec<String> = u.members.iter().map(|m| m.name.to_string()).collect();
                entries.push((
                    name.clone(),
                    type_meta_literal(&name, "Union", "None", "&[]", &possible),
                ));
            }
            // Scalars, enums and input objects need no TypeMeta: field kinds
            // are resolved at codegen and inputs never appear in responses.
            _ => {}
        }
    }

    if !errors.is_empty() {
        panic!(
            "schema shape errors:\n  - {}\n(fix the schema — see the presence-of-id convention in this build script's docs)",
            errors.join("\n  - ")
        );
    }

    entries.sort_by(|a, b| a.0.cmp(&b.0)); // binary-searchable

    // Namespace persisted query records by the complete query contract:
    // normalization-relevant outputs plus every reachable argument/input
    // shape. Mutation-only SDL and documentation remain excluded.
    let reachable = reachable_query_types(&schema, &query_root, &implementors);
    let mut hasher = Sha256::new();
    hasher.update(query_root.as_bytes());
    for (name, literal) in &entries {
        if reachable.contains(name) {
            hasher.update(name.as_bytes());
            hasher.update(literal.as_bytes());
        }
    }
    for name in &reachable {
        let Some(ty) = schema.types.get(name.as_str()) else {
            continue;
        };
        match ty {
            ExtendedType::Object(object) => {
                hash_field_arguments(&mut hasher, object.fields.iter());
            }
            ExtendedType::Interface(interface) => {
                hash_field_arguments(&mut hasher, interface.fields.iter());
            }
            ExtendedType::InputObject(input) => {
                hasher.update(b"input\0");
                hasher.update(name.as_bytes());
                for (field_name, field) in &input.fields {
                    hasher.update(field_name.as_str().as_bytes());
                    hasher.update(type_signature(&field.ty).as_bytes());
                    if let Some(default) = &field.default_value {
                        hasher.update(format!("{:?}", &**default).as_bytes());
                    }
                    hasher.update(b"\0");
                }
            }
            ExtendedType::Enum(enum_type) => {
                hasher.update(b"enum\0");
                hasher.update(name.as_bytes());
                let values = enum_type
                    .values
                    .keys()
                    .map(ToString::to_string)
                    .collect::<BTreeSet<_>>();
                for value in values {
                    hasher.update(value.as_bytes());
                    hasher.update(b"\0");
                }
            }
            ExtendedType::Scalar(_) => {
                hasher.update(b"scalar\0");
                hasher.update(name.as_bytes());
            }
            ExtendedType::Union(_) => {}
        }
    }
    let schema_hash = hex_string(&hasher.finalize());

    let mut out = String::new();
    writeln!(out, "// @generated by cache-core/build.rs — do not edit.").unwrap();
    writeln!(out, "pub static QUERY_ROOT_TYPE: &str = {query_root:?};").unwrap();
    match &mutation_root {
        Some(name) => writeln!(
            out,
            "pub static MUTATION_ROOT_TYPE: Option<&str> = Some({name:?});"
        )
        .unwrap(),
        None => writeln!(out, "pub static MUTATION_ROOT_TYPE: Option<&str> = None;").unwrap(),
    }
    writeln!(out, "pub static SCHEMA_HASH: &str = {schema_hash:?};").unwrap();
    writeln!(out, "pub static TYPES: &[TypeMeta] = &[").unwrap();
    for (_, literal) in &entries {
        writeln!(out, "    {literal},").unwrap();
    }
    writeln!(out, "];").unwrap();

    let out_path = Path::new(&std::env::var("OUT_DIR").unwrap()).join("schema_meta.rs");
    std::fs::write(out_path, out).unwrap();
}

fn reachable_query_types(
    schema: &Schema,
    query_root: &str,
    implementors: &BTreeMap<String, BTreeSet<String>>,
) -> BTreeSet<String> {
    let mut reachable = BTreeSet::new();
    let mut pending = vec![query_root.to_owned()];

    while let Some(name) = pending.pop() {
        if !reachable.insert(name.clone()) {
            continue;
        }
        let Some(ty) = schema.types.get(name.as_str()) else {
            continue;
        };
        match ty {
            ExtendedType::Object(object) => {
                push_field_types(schema, object.fields.iter(), &mut pending);
            }
            ExtendedType::Interface(interface) => {
                push_field_types(schema, interface.fields.iter(), &mut pending);
                pending.extend(
                    implementors
                        .get(&name)
                        .into_iter()
                        .flat_map(|types| types.iter().cloned()),
                );
            }
            ExtendedType::Union(union) => {
                pending.extend(union.members.iter().map(|member| member.name.to_string()));
            }
            ExtendedType::InputObject(input) => {
                for field in input.fields.values() {
                    push_reachable_type(schema, &field.ty, &mut pending);
                }
            }
            _ => {}
        }
    }
    reachable
}

fn push_field_types<'a>(
    schema: &Schema,
    fields: impl Iterator<
        Item = (
            &'a apollo_compiler::Name,
            &'a apollo_compiler::schema::Component<apollo_compiler::ast::FieldDefinition>,
        ),
    >,
    pending: &mut Vec<String>,
) {
    for (_, field) in fields {
        push_reachable_type(schema, &field.ty, pending);
        for argument in &field.arguments {
            push_reachable_type(schema, &argument.ty, pending);
        }
    }
}

fn push_reachable_type(
    schema: &Schema,
    ty: &apollo_compiler::ast::Type,
    pending: &mut Vec<String>,
) {
    let Ok((name, ..)) = flatten_type(ty) else {
        return;
    };
    if schema.types.contains_key(name.as_str()) {
        pending.push(name);
    }
}

fn hash_field_arguments<'a>(
    hasher: &mut Sha256,
    fields: impl Iterator<
        Item = (
            &'a apollo_compiler::Name,
            &'a apollo_compiler::schema::Component<apollo_compiler::ast::FieldDefinition>,
        ),
    >,
) {
    for (field_name, field) in fields {
        for argument in &field.arguments {
            hasher.update(b"argument\0");
            hasher.update(field_name.as_str().as_bytes());
            hasher.update(argument.name.as_str().as_bytes());
            hasher.update(type_signature(&argument.ty).as_bytes());
            if let Some(default) = &argument.default_value {
                hasher.update(format!("{:?}", &**default).as_bytes());
            }
            hasher.update(b"\0");
        }
    }
}

fn type_signature(ty: &apollo_compiler::ast::Type) -> String {
    use apollo_compiler::ast::Type;
    match ty {
        Type::Named(name) => name.to_string(),
        Type::NonNullNamed(name) => format!("{name}!"),
        Type::List(inner) => format!("[{}]", type_signature(inner)),
        Type::NonNullList(inner) => format!("[{}]!", type_signature(inner)),
    }
}

/// Presence-of-id convention: a type with an `id` field is keyed by it; the
/// field must be a non-null, non-list `ID`. Types without `id` are embedded.
fn key_literal<'a>(
    name: &str,
    is_operation_root: bool,
    fields: impl Iterator<
        Item = (
            &'a apollo_compiler::Name,
            &'a apollo_compiler::schema::Component<apollo_compiler::ast::FieldDefinition>,
        ),
    >,
    errors: &mut Vec<String>,
) -> String {
    let id_field = fields.into_iter().find(|(fname, _)| fname.as_str() == "id");
    let Some((_, fdef)) = id_field else {
        return "None".into();
    };
    if is_operation_root {
        errors.push(format!(
            "operation root `{name}` must not expose an `id` field"
        ));
        return "None".into();
    }
    match flatten_type(&fdef.ty) {
        Ok((named, false, false, _)) if named == "ID" => r#"Some(&["id"])"#.into(),
        _ => {
            errors.push(format!(
                "`{name}.id` must be `ID!` (non-null, non-list) to satisfy the presence-of-id convention; rename it if it is not this object's identity"
            ));
            "None".into()
        }
    }
}

fn type_meta_literal(
    name: &str,
    kind: &str,
    key_literal: &str,
    fields_literal: &str,
    possible: &[String],
) -> String {
    let possible_list = possible
        .iter()
        .map(|p| format!("{p:?}"))
        .collect::<Vec<_>>()
        .join(", ");
    format!(
        "TypeMeta {{ name: {name:?}, kind: TypeKind::{kind}, key_fields: {key_literal}, fields: {fields_literal}, possible_types: &[{possible_list}] }}"
    )
}

fn fields_literal<'a>(
    schema: &Schema,
    fields: impl Iterator<
        Item = (
            &'a apollo_compiler::Name,
            &'a apollo_compiler::schema::Component<apollo_compiler::ast::FieldDefinition>,
        ),
    >,
    errors: &mut Vec<String>,
) -> String {
    let mut parts = Vec::new();
    for (fname, fdef) in fields {
        let (named, nullable, list, item_nullable) = match flatten_type(&fdef.ty) {
            Ok(t) => t,
            Err(e) => {
                errors.push(format!("field `{fname}`: {e}"));
                continue;
            }
        };
        let kind = match schema.types.get(named.as_str()) {
            Some(ExtendedType::Object(_))
            | Some(ExtendedType::Interface(_))
            | Some(ExtendedType::Union(_)) => "Composite",
            Some(ExtendedType::Enum(_)) => "Leaf",
            Some(ExtendedType::Scalar(_)) => {
                if matches!(
                    named.as_str(),
                    "Int" | "Float" | "String" | "Boolean" | "ID"
                ) {
                    "Leaf"
                } else {
                    "OpaqueScalar" // e.g. JSON
                }
            }
            Some(ExtendedType::InputObject(_)) | None => {
                errors.push(format!("field `{fname}` has non-output type `{named}`"));
                continue;
            }
        };
        parts.push(format!(
            "FieldMeta {{ name: {:?}, ty: FieldType {{ name: {:?}, kind: FieldKind::{kind}, nullable: {nullable}, list: {list}, item_nullable: {item_nullable} }} }}",
            fname.to_string(),
            named,
        ));
    }
    format!("&[{}]", parts.join(", "))
}

/// Flattens GraphQL type wrapping to (named, nullable, list, item_nullable).
/// Nested lists are rejected — extend `FieldType` if the schema ever needs
/// them.
fn flatten_type(ty: &apollo_compiler::ast::Type) -> Result<(String, bool, bool, bool), String> {
    use apollo_compiler::ast::Type;
    let (inner, nullable, list) = match ty {
        Type::Named(n) => return Ok((n.to_string(), true, false, false)),
        Type::NonNullNamed(n) => return Ok((n.to_string(), false, false, false)),
        Type::List(inner) => (inner, true, true),
        Type::NonNullList(inner) => (inner, false, true),
    };
    match inner.as_ref() {
        Type::Named(n) => Ok((n.to_string(), nullable, list, true)),
        Type::NonNullNamed(n) => Ok((n.to_string(), nullable, list, false)),
        Type::List(_) | Type::NonNullList(_) => Err("nested lists are not supported".into()),
    }
}

fn hex_string(bytes: &[u8]) -> String {
    bytes.iter().map(|b| format!("{b:02x}")).collect()
}
