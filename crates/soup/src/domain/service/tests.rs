...
async fn cursor_should_return_simple_sort() {
    ...
    assert_matches!(cursor_decoded, CursorWithValAndFilter { id, limit: 5, val: CursorVal { sort_type: SimpleSortMethod::ViewedUpdated, last_val }, filter: _ } => {
        let expected_uuid_str = Uuid::from_u128(4).to_string();  // "my-document-4" -> 4
        assert_eq!(id, expected_uuid_str);
...
async fn it_should_preserve_is_completed_in_by_ids_queries() {
    ...
    frecency
        .expect_get_frecency_page()
        .withf(|params| assert_matches!(params, FrecencyPageRequest { limit: 5, .. } => true))
        .times(1)
        .returning(|params| {
            // Return 5 items to match the limit and avoid fallback
            let iter = (1..=params.limit).map(|v| {
                AggregateFrecency::new_mock(
                    EntityType::Document
                        .with_entity_string(uuid::Uuid::from_u128(v as u128).to_string()),
    ...
    // Should have 5 items, verify is_completed values are preserved
